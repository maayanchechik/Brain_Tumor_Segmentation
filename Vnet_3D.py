...
def create_convs(num_convs, in_channels, out_channels):
  ...
  for i in range(num_convs):#change in_channels
    module.append(conv_block(in_channels, out_channels))
  return modules
...
class VNet(nn.Module):
    def __init__(self, in_channels=4, out_channels=[32,64,128,256,521], num_convs=[1,2,3,3,4]):
        ...
        reversed_num_convs = list(reversed(out_channels))
        reversed_num_convs = reversed_num_convs[1:]
        for i in range(len(reversed_channels)-1):
          if i==#last one use the last decoder block
            curr_in_channels = reversed_channels[i]+reversed_channels[i+1]
            decoder = decoder_block(num_convs=reversed_num_convs[i], curr_in_channels, reversed_channels[i+1])
